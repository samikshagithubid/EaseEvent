...
document.getElementById('uploadInput').addEventListener('change', function(event) {
    ...
    Array.from(files).forEach(file => {
        if (file.size > 20 * 1024 * 1024) { // 20MB
            ...
            uploadedFilesList.push(file.name); // Add file name to the list
...
document.getElementById('downloadPDF').addEventListener('click', function(event) {
    console.log("JavaScript file loaded successfully"); // Add this line at the start of your JS file
    ...
    if (uploadedFilesList.length > 0) {
        ...
        uploadedFilesList.forEach((fileName, index) => {
            pdf.text(fileName, 10, 70 + (index * 10));
        });
    } else {
